...
public class RegexRepository {
    public static final String klassDefinitionRegex = "(public )?((abstract )?class|interface) \\w+(<.*>)?( extends \\w+(<.*>)?)?( implements \\w+(<.*>)?\\s?(,\\s?\\w+(<.*>)?\\s?)*)?\\s?[{]";//basically: (public )?class <className>( extends <parent>)?( implements <interface>, <interface>...)? {
